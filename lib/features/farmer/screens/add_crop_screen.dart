...
class _AddCropScreenState extends State<AddCropScreen> {
  ...
  Future<void> _pickAndProcessImage(ImageSource source,
    ...
    try {
      ...
      if (mounted) _showError("Permission Denied: ${e.message}");
    } catch (e) {
  ...
  Future<void> _analyzeImageWithAI(File imageFile) async {
    ...
    try {
      ...
      final prompt = TextPart('''
        ...
        Verify if the image contains an agricultural crop, fruit, vegetable, plant, or farm produce.
        
        If it is NOT a crop (like a human, a car, or furniture), return EXACTLY:
        {"is_crop": false}
        
        If it IS a crop, return exactly this JSON:
        {
          "is_crop": true,
          "category": "Match one: Vegetables, Fruits, Grains & Cereals, Spices & Plantation, Medicinal & Aromatic, Flowers, Pulses",
          "crop_name": "Accurate English name (e.g. Tomato, Onion)",
          "variety": "Specific Indian variety if known, else 'Local / Desi'",
          "grade": "Match one: Grade A (Premium), Grade B (Standard), Grade C (Fair), Organic Certified",
          "estimated_weight_kg": "Number only",
          "days_until_harvest": "0 if harvested, else estimate remaining days (Number only)",
          "short_note": "1 short professional sentence about the crop quality."
        }
      ''');
